...
def view():
       ...
       cur.execute('SELECT * FROM fee order by date desc')
       row = cur.fetchall()
       return row
...
def search(recpt = ' ', name = ' ', admsn = ' ', date = ' ', branch = ' ', sem = ' ', total = ' ', paid = ' ', due = ' '):
       ...
       cur.execute('SELECT * FROM fee WHERE  recpt = ? OR name = ? OR admsn = ? OR date = ? OR branch = ? OR sem = ? OR total = ? OR paid = ? OR due = ? order by date desc',(recpt,name,admsn,date,branch,sem,total,paid,due))
       row = cur.fetchall()
       return row
